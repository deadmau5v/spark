...
make_release $VERSION $FORCE="":
   sed -i 's/^version = .*/version = "'$VERSION'"/g' spark-cli/Cargo.toml spark/Cargo.toml
   ...
   git add spark/Cargo.* spark-cli/Cargo.* Cargo.*
...
docker_release $TAG:
  ...
  #~/.depot/bin/depot build --project v4z5w7md33 --platform linux/arm/v7,linux/arm64,linux/amd64 -t ghcr.io/deadmau5v/spark:$TAG -t ghcr.io/deadmau5v/spark:latest --push .
  ...
  docker buildx build \
    ...
    --cache-from type=registry,ref=ghcr.io/deadmau5v/spark:cache \
    --cache-to type=registry,mode=max,image-manifest=true,oci-mediatypes=true,ref=ghcr.io/deadmau5v/spark:cache \
    -t ghcr.io/deadmau5v/spark:$TAG \
    -t ghcr.io/deadmau5v/spark:latest \
    --push .
